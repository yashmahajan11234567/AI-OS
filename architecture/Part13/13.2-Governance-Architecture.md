# Part 13.2: Governance Architecture

> **Purpose:** Define the logical architecture of governance components, their interaction patterns, and lifecycle within the AI-OS architecture.
> **Foundation:** Components are specified in [components.md](./components.md) and integrate with AI-OS Parts 1–12 as described in §4 of the [Architecture Overview](./13.1-Architecture-Overview.md).
> **Conformance:** Aligns with Part 12 governance event integration contract and the `governance` namespace registered in Part 12 §25.

---

## 1. Component Architecture Model

The Part 13 governance architecture is composed of **16 logical components**, organized into a **three-tier hierarchy**. Components are purely logical — physical deployment topology is out of scope for Part 13.
...
└── (Governance results reported to operating context)
```
...
```text
                    G governance Manager (G-00)
                    orchestrates, dispatches, coordinates
┌────────┬─────────┬────────────┐
│        │         │            │
▼        ▼         ▼
Policy Manager (G-01) │ Governance Registry (G-03) │ Governance Council (G-04)
CRUD, lifecycle,  │ Canonical state, query, │ Charter, committees,
version  │ lookup, snapshot │ audits/view
│
▼
┌─────────────────────────────────────────┐
...
Eval Engine │  Authority │  Risk register,
  (G-02) │  Manager │  treatment, tolerance
       ...
       ▼
┌───────────────────────────┐
│    Control Manager (G-13) │ Control set, testing,
│   Control design, testing,  │ effectiveness evidence
│   effectiveness  │
└──────────────┬────────────┘
               │
┌──────────────┼──────────────────────────────────┐
...
└────────┬─────────┘             └────────┬─────────┘
         ...
         ▼                              ▼
┌─────────────────────────┐   ┌─────────────────────────┐
│    Accountability       │   │    Audit Mgr   │  Conformance Mgr │  Governance
│    Manager (G-10)       │   │  (G-09)       │  (G-15) │ Event Manager
│  Principal-actor-subject │   │  Evidence,   │  Snapshot,    │  (G-14)
│  link, causal chain  │   │  findings │  pass/fail │  Events, classification
└─────────────────────────┘   └─────────────────────────┘
```
...
- **Governance Event Manager (G-14)** is the nervous system — every component emits governance events through it; events are routed to G-09 for persistence and to consumers for processing.
...
The following sections provide the detailed specification for each component. Each component specification follows a consistent structure.

### 4.1 G-00 — Governance Manager

#### Purpose
Orchestrate and manage the lifecycle of all governance sessions. Dispatch work to appropriate components based on governance trigger events. Own the governance session state machine and ensure every session reaches a terminal state with an outcome (decision, escalation, or error).

#### Responsibilities
- Initialize governance sessions from trigger events.
- Route evaluation requests to G-02, authority resolution to G-05, approval routing to G-12.
- Maintain session correlation context for traceability.
- Coordinate multi-stage governance flows (e.g., policy evaluation → authority check → approval).
- Ensure principal chain binding for every session (via G-10).
- Emit session lifecycle events (opened, transitioned, completed, failed).
- Handle session escalation on component failure.

#### Non-Responsibilities
- Does not create or modify policy — delegates to G-01.
- Does not evaluate policy conditions — delegates to G-02.
- Does not hold policy artifacts — delegates to G-03.
- Does not grant authority — delegates to G-05.
- Does not make approval decisions — delegates to G-12.

#### Inputs
- Governance trigger event (from any operating layer, via G-14).
- Principal identity context (from G-10).
- Operating context envelope (opaque to G-00).
...
- **Outbound:** `requestEvaluation(policySet, context) → G-02`; `resolveAuthority(req) → G-05`; `emit(event) → G-14`.
...
### 4.3 G-02 — Policy Evaluation Engine
...
#### Inputs
- Evaluation request: `{sessionId, context, policyScope, grain}` from G-00.
...
- **Outbound:** `lookupSnapshot(policyId, version) → G-03`; `isExceptionActive(exceptionId) → G-11`; `emit(event) → G-14`; `recordPrincipal(actor, principal) → G-10`.
...
- **Outbound:** `registerCharter(charter) → G-03`; `proposePolicy(proposal) → G-01`; `approveDecision(decision) → G-12`; `emit(event) → G-14`.
...
### 4.6 G-05 — Decision Authority Manager
...
### 4.7 G-06 — Delegation Authority Manager
...
- Does not set risk tolerance — tolerances come from policy and council (G-04).
...
- Risk tolerance parameters (from G-03 policy snapshot).
...
- **Outbound:** `queryTolerance(scope) → G-03`; `routeForApproval(proposal) → G-12`; `emit(event) → G-14`; `shareRiskPosture() → G-08, G-15`.
- **Query:** `getRisk(riskId)`; `getRiskRegister(scope)`; `getRiskPosture(scope)`.

#### Dependencies
- G-03 (risk policy artifacts, tolerance parameters).
...
#### Authority
Does not set tolerance or treatment — records risk, proposes treatment, enforces review cycle.

#### Ownership
- Risk Management function (risk register ownership).
- Governance Product (risk taxonomy).
- Council (risk tolerance charter).
...
### 4.11 G-10 — Accountability Manager
...
- **Outbound:** `notifyApproved(requestId, action) → appropriate component`; `notifyRejected(requestId, rationale)`; `emit(event) → G-14`.
...
### 4.15 G-14 — Governance Event Manager
...
#### Governance Constraints
Every governance action MUST emit at least one event before completing. Event suppression is prohibited. Schema changes requires audit Product approval and charter compliance verification.
...
### 5.1 Authoritative Flow: Policy Change → Evaluation Update
...
│  ┌─────────────┐                         ┌──────────────┐
│  │  G-00       │────────────────────────▶│   G-09       │
│  │Governance Mgr│   auditRecord           │   Audit Mgr   │
│  │   Mgr       │                         └──────────────┘
│  └─────────────┘
│
└──────────────────────────────────────────────────────────────────────
...
### 5.2 Sequence: Exception Request and Application
...
### 5.3 Sequence: Approval Lifecycle
...
## 6. Specification Notes

### 6.1 Relation to "Parts" in the AI-OS Specification
...
Tier 0 components must initialize before any governance flow:

1. **G-03** initializes first (canonical state).
2. **G-01** loads initial policy package into G-03.
3. **G-00** activates after G-01 and G-03 are available.
4. **Tier 1 components** initialize in topological order based on dependencies.
5. **G-14** accepts events from all components once G-03 schema is loaded.
6. **Tier 2 components (G-13, G-15)** initialize last, after stable execution evidence exists.

### 6.3 Authority Boundary Summary

| Component | Authority Boundary | Cannot |
|-----------|-------------------|---------------------|
| G-00 | Session lifecycle | Bypass evaluation results |
| G-01 | Policy text/package | Approve own packages |
| G-02 | Runtime evaluation | Modify policy, execute enforcement |
| G-03 | Artifact validity | Unilaterally retire artifacts |
| G-04 | Charter/committees | Override published policy |
| G-05 | Authority grants | Delegate authority |
| G-06 | Delegation chains | Grant initial authority |
| G-07 | Risk records | Set tolerance or treatment |
| G-08 | Obligation tracking | Create or modify obligations |
| G-09 | Audit immutability | Delete or edit entries |
| G-10 | Principal bindings | Verify actor authenticity |
| G-11 | Exception issuance | Authorize without G-05 authority |
| G-12 | Approval routing | Override authority decisions |
| G-13 | Control design | Evaluate policy or set obligations |
| G-14 | Event routing | Suppress events or override schema |
| G-15 | Conformance assessment | Alter baselines or make treatment decisions |

### 6.4 Communication Model

Part 13 uses a **hybrid communication model** — not a blanket event-only architecture.

**Governance events** are used for:
- Asynchronous state propagation across components.
- Audit trail creation (all events flow through G-14 to G-09).
- Session correlation and causal chain reconstruction.
- Broadcast notifications to multiple subscribers.

**Direct synchronous interfaces** are used for:
- Queries requiring immediate responses (e.g., `evaluate(request) → decision`, `resolveAuthority(req) → decision`).
- Command operations where the caller needs confirmation before proceeding (e.g., `registerCharter(charter)`, `submitForApproval(request)`).
- Read-only lookups from shared state (e.g., `getArtifact(id, version)` from G-03).

**G-14 routing responsibilities:**
G-14 receives every governance event emitted by any component. It classifies, persists, and routes events to subscribers. G-09 is the primary persistence target: all events are written to the audit trail through G-14. Components do not write audit records directly — they emit events, and G-14 delivers them to G-09.

**G-09 audit evidence responsibility:**
G-09 remains responsible for audit evidence management. It receives events via G-14, seals immutable audit entries, maintains integrity hashes, and produces audit manifests. No component bypasses G-14 to write audit records.

### 6.5 Component Interaction Matrix

The following matrix covers the primary interactions. Secondary interactions are documented in each component's individual specification.

| Caller | Interface / Event | Target | Purpose |
|--------|------------------|--------|---------|
| Any operating layer | `GovernanceTrigger` event | G-00 via G-14 | Initiate governance session |
| G-00 | `requestEvaluation(policySet, context)` | G-02 | Request policy evaluation |
| G-00 | `resolveAuthority(req)` | G-05 | Resolve decision authority |
| G-00 | `emit(event)` | G-14 | Emit session lifecycle events |
| G-00 | `queryGovernanceState(sessionId)` | G-00 | Query governance session state |
| G-01 | `publishPackage(version, scope, content)` | G-03 | Register published policy package |
| G-01 | `queueApproval(policyId, diff)` | G-12 | Submit policy for approval |
| G-01 | `emit(event)` | G-14 | Emit policy lifecycle events |
| G-01 | `resolvePolicy(policyId, version)` | G-03 | Lookup policy package |
| G-02 | `evaluate(request)` | G-02 | Evaluate policy conditions |
| G-02 | `lookupSnapshot(policyId, version)` | G-03 | Resolve policy snapshot |
| G-02 | `isExceptionActive(exceptionId)` | G-11 | Check exception state |
| G-02 | `getActiveChainForActor(actorId)` | G-06 | Validate delegation chain |
| G-02 | `recordPrincipal(actor, principal)` | G-10 | Bind actor to principal |
| G-02 | `emit(event)` | G-14 | Emit evaluation events |
| G-03 | `registerArtifact(artifact, metadata)` | G-03 | Register governance artifact |
| G-03 | `getArtifact(id, version)` | G-03 | Retrieve artifact |
| G-03 | `getSnapshot(scope, effectiveAsOf)` | G-03 | Retrieve compiled snapshot |
| G-04 | `registerCharter(charter)` | G-03 | Register council charter |
| G-04 | `proposePolicy(proposal)` | G-01 | Submit policy proposal |
| G-04 | `approveDecision(decision)` | G-12 | Submit for approval |
| G-04 | `emit(event)` | G-14 | Emit council events |
| G-05 | `resolveAuthority(request)` | G-05 | Resolve authority grant |
| G-05 | `checkDelegationChain(delegationId)` | G-06 | Validate delegation chain |
| G-05 | `emit(event)` | G-14 | Emit authority events |
| G-06 | `createDelegation(request)` | G-06 | Create delegation grant |
| G-06 | `revokeDelegation(delegationId, reason)` | G-06 | Revoke delegation |
| G-06 | `validateChain(actorId, action, target)` | G-05 | Provide chain validation to authority |
| G-06 | `emit(event)` | G-14 | Emit delegation events |
| G-07 | `submitRisk(risk)` | G-07 | Submit risk identification |
| G-07 | `submitTreatment(treatment)` | G-07 | Submit treatment proposal |
| G-07 | `routeForApproval(proposal)` | G-12 | Route treatment for approval |
| G-07 | `shareRiskPosture()` | G-08, G-15 | Share risk posture |
| G-07 | `queryTolerance(scope)` | G-03 | Lookup tolerance parameters |
| G-07 | `emit(event)` | G-14 | Emit risk events |
| G-08 | `importObligations(source, scope)` | G-08 | Import obligations |
| G-08 | `updateEvidence(obligationId, evidence)` | G-08 | Update evidence |
| G-08 | `publishBaseline(baseline)` | G-03 | Publish compliance baseline |
| G-08 | `notifyConformanceEngine(baseline)` | G-15 | Notify conformance engine |
| G-08 | `emit(event)` | G-14 | Emit compliance events |
| G-09 | `acceptEvent(event)` | G-09 | Accept event for audit record |
| G-09 | `requestAuditManifest(scope, range)` | G-09 | Request audit manifest |
| G-09 | `packageEvidence(scope, range)` | G-09 | Package evidence for external use |
| G-09 | `notifyIntegrityBreach(auditId)` | G-00 | Alert on integrity breach |
| G-09 | `queryAudit(scope, range, filters)` | G-09 | Query audit records |
| G-10 | `bindActor(actor, principal, evidence)` | G-10 | Bind actor to principal |
| G-10 | `recordAction(action, principalBindings)` | G-10 | Record action with bindings |
| G-10 | `resolvePrincipalChain(actorId)` | G-10 | Resolve principal chain |
| G-10 | `emit(event)` | G-14 | Emit accountability events |
| G-11 | `requestException(request)` | G-11 | Request exception |
| G-11 | `renewException(exceptionId)` | G-11 | Renew exception |
| G-11 | `notifyEvaluator(exceptionId)` | G-02 | Inject exception into evaluation |
| G-11 | `reportCompliance()` | G-08, G-15 | Report exception compliance |
| G-11 | `emit(event)` | G-14 | Emit exception events |
| G-12 | `submitForApproval(request)` | G-12 | Submit for approval |
| G-12 | `recordDecision(requestId, decision)` | G-12 | Record approval decision |
| G-12 | `notifyApproved(requestId, action)` | appropriate component | Notify approval outcome |
| G-12 | `notifyRejected(requestId, rationale)` | requester | Notify rejection |
| G-12 | `emit(event)` | G-14 | Emit approval events |
| G-13 | `registerControl(control)` | G-13 | Register control |
| G-13 | `submitTestResult(controlId, test)` | G-13 | Submit test result |
| G-13 | `queryControl(testId)` | root | Query control lineage |
| G-13 | `shareEvidence()` | G-09, G-08, G-15 | Share control evidence |
| G-13 | `emit(event)` | G-14 | Emit control events |
| G-14 | `emit(event)` | G-14 | Receive events from all components |
| G-14 | `getEventsForSession(sessionId)` | G-14 | Query events by session |
| G-14 | `getEventsByType(type, range)` | G-14 | Query events by type |
| G-14 | `routeEvent(event, target)` | G-14 | Route event to target |
| G-15 | `requestConformanceEvaluation()` | G-15 | Request conformance evaluation |
| G-15 | `registerBaseline(baseline, source)` | G-15 | Register baseline |
| G-15 | `notifyGap(gap)` | G-12 | Route gap for treatment |
| G-15 | `publishReport(report)` | G-04, G-09 | Publish conformance report |
| G-15 | `alertConformanceBreach(breach)` | G-00 | Alert on breach |
| G-15 | `emit(event)` | G-14 | Emit conformance events |
| All components | `emit(event)` | G-14 | Emit governance event for routing |
| G-14 | routed events | G-09 | Persist events as audit records |
| G-14 | routed events | G-00 | Correlate session events |
| G-14 | routed events | G-07, G-08 | Root-cause analysis |

---

## 7. Cross-References

- **13.1 Architecture Overview** — component architecture model, governance invariants.
- **13.3 Policy Architecture** — policy evaluation, policy lifecycle.
- **13.4 Decision Authority and Delegation** — authority model, delegation chains.
- **13.5 Governance Councils and Committees** — council operations, committee structure.
- **13.6 Risk and Compliance Governance** — risk treatment, compliance baselines.
- **13.7 Agent and Capability Governance** — agent authority, capability evaluation.
- **13.8 Workflow and Execution Governance** — workflow execution, enforcement integration.
- **13.9 Data and Knowledge Governance** — data controls, knowledge policy.
- **13.10 Security and Trust Governance** — trust boundaries, security events.
- **13.11 Auditability and Accountability** — audit trail, accountability tracing.
- **13.12 Governance Invariants and Conformance** — conformance scoring, invariant enforcement.
- **adrs.md** — Component ADRs C13.COMP.01–C13.COMP.10.
- **governance-events.md** — 28 governance events across 8 aggregates.
- **schemas.md** — 11 formal governance schemas.
- **dependency-map.md** — component-to-part dependencies.
- **review-checklist.md** — component conformance verification items.

---

*← [Part 13.1: Architecture Overview](./13.1-Architecture-Overview.md) | [Part 13.3: Policy Architecture](./13.3-Policy-Architecture.md) →*