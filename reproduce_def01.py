...
def test_def01_reproduction():
    ...
    try:
        ...
        # Create MCPManager which loads configs from config/mcp/ (normal production path)
        print("Creating MCPManager (loading configs from config/mcp/)...")
        mcp_manager = MCPManager()

        print(f"Loaded {len(mcp_manager._servers)} server configs")

        # Now try to validate one of the servers through SecurityManager (this should trigger DEF-01)
