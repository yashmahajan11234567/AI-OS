...
| E2E | ✅ | ✅ M8 | M8-T1 Hermes ACP real subprocess test exercised genuine subprocess boundary with real ACP communication; proper authority boundaries and gated behavior verified (independent QA verified) |
...
| Learning | ✅ | ✅ M9 | Lesson validation and promotion semantics exist; LearningValidated event correctly integrated; PlanningService uses validated learning per authoritative definition; validation/promotion deterministic with regression protection (independent QA verified) |
...
| Graphify | ✅ | ✅ M8 | GraphifyAdapter exists; real MCP connection path exists with genuine subprocess/process boundary; MCP handshake/communication is genuine; real-MCP test properly gated; no fabricated success; credentials not hardcoded; Graphify remains advisory/untrusted; deterministic fallback/mock behavior not incorrectly counted as real-MCP acceptance (independent QA verified) |
...
| Criterion | Status | Target | Evidence |
|-----------|--------|--------|----------|
| Model access | ✅ | ✅ M9 | ModelRouter exists; FreeLLMAPI adapter exists; _call_model genuinely dispatches to registered FreeLLMAPI provider when configured; provider registration real; runtime configuration used; credentials not hardcoded; unavailable-provider behavior fails safely; DEV/TEST restrictions respected; test proves actual dispatch rather than merely mocking router; gated real-provider path verified when environment permits (independent QA verified) |
...
| Monitoring | ✅ | ✅ M10 | Health endpoint exists; readiness endpoint exists; endpoint behavior deterministic; healthy/degraded/unhealthy conditions represented correctly; endpoint uses existing HealthManager; no parallel health authority introduced; sensitive configuration/secrets not exposed; HTTP tests exercise endpoints; lifecycle/authority semantics remain intact (independent QA verified) |
