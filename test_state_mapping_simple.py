...
def test_state_mapping_priority():
    ...
    print("1. Testing HUMAN_ESCALATION_REQUIRED -> ESCALATING (Priority 1)")
    ...
    print("2. Testing COMPLETE -> COMPLETE (Priority 2)")
    context = MascotStateContext(
    ...
    # Priority 3: UNHEALTHY -> ESCALATING
    print("3. Testing UNHEALTHY -> ESCALATING (Priority 3)")
    context = MascotStateContext(
    ...
    print("4. Testing DEGRADED health status -> ESCALATING (Priority 4)")
    context = MascotStateContext(
    ...
    print("5. Testing DEGRADED lifecycle state -> ESCALATING (Priority 4)")
    context = MascotStateContext(
    ...
    print("6. Testing Active workflow -> workflow state (Priority 5)")
    test_cases = [
    ...
    print("8. Testing shutdown/terminated -> IDLE (Priority 7)")
    shutdown_tests = [
    ...
    print("ALL STATE MAPPING PRIORITY TESTS PASSED")
    return True
...
def test_state_properties():
    ...
    # Test should_animate
    print("1. Testing should_animate property")
    ...
    print("2. Testing is_interruptible property")
    interrupt_tests = [
        # (current, incoming, expected_result, description)
        (MascotState.IDLE, MascotState.PLANNING, True, "IDLE -> PLANNING"),
        (MascotState.PLANNING, MascotState.EXECUTING, True, "PLANNING -> EXECUTING"),
        (MascotState.EXECUTING, MascotState.EXECUTING, False, "EXECUTING -> EXECUTING"),
        (MascotState.IDLE, MascotState.ESCALATING, True, "IDLE -> ESCALATING"),
        (MascotState.PLANNING, MascotState.ESCALATING, True, "PLANNING -> ESCALATING"),
        (MascotState.IDLE, MascotState.COMPLETE, True, "IDLE -> COMPLETE"),
        (MascotState.EXECUTING, MascotState.COMPLETE, True, "EXECUTING -> COMPLETE"),
        (MascotState.EXECUTING, MascotState.IDLE, True, "EXECUTING -> IDLE"),
    ]
