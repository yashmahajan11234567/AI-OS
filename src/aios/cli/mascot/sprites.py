"""
AI-OS Cyber Turtle Sprite Definitions - Pixel-Art Asset Edition.
...
The turtle is a presentation/control surface only - it never mutates AI-OS state.
"""
...
from aios.cli.mascot.halfblock import RenderMode, render_state, HalfBlockRasterizer


class MascotSpriteState(str, Enum):
    """Logical mascot states that map to sprite frames."""
    IDLE = "idle"
    PLANNING = "planning"
    EXECUTING = "executing"
    REVIEWING = "reviewing"
...
class MascotFrame:
    """Single mascot animation frame - rendered string output."""
    lines: List[str]
    width: int
    height: int
...
class MascotAnimation:
    """Animation sequence for a mascot state."""
...
class MascotSprites:
    """Centralized sprite data for all mascot states - backed by pixel-art assets."""

    # Cache for rendered frames per mode
    _RENDERED_CACHE: Dict[tuple, MascotFrame] = {}

    @classmethod
    def _render_asset_frame(cls, state: str, frame_index: int, mode: RenderMode) -> MascotFrame:
        """Render an asset frame to MascotFrame for the given mode."""
        cache_key = (state, frame_index, mode)
        if cache_key in cls._RENDERED_CACHE:
            return cls._RENDERED_CACHE[cache_key]

        asset_frame = MascotAssets.get_frame(state, frame_index)
        # Use asset dimensions (each char cell = 2 pixel rows)
        asset_width, asset_height = MascotAssets.get_dimensions(state)
    ...
    @classmethod
    def get_animation(cls, state: MascotSpriteState) -> MascotAnimation:
        """Get animation for a state."""
        asset_state = state.value.upper()
        frame_count = MascotAssets.get_frame_count(asset_state)
        frames = [
        ...
        return MascotAnimation(frames=frames, loop=loop, frame_delay=delay)
    ...
    @classmethod
    def get_dimensions(cls, state: MascotSpriteState, narrow: bool = False, monochrome: bool = False) -> tuple[int, int]:
        """Get dimensions for a state."""
        asset_state = state.value.upper()
        asset_w, asset_h = MascotAssets.get_dimensions(asset_state)
        char_w = asset_w
        char_h = (asset_h + 1) // 2
...
# Validate on import
_SPRITE_ERRORS = MascotSprites.validate_all()
if _SPRITE_ERRORS:
    raise RuntimeError(f"Mascot sprite validation failed: {_SPRITE_ERRORS}")