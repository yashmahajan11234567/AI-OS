...
from aios.cli.owl.sprites import OwlSprites, OwlAnimation, OwlSpriteState
...
class OwlAnimator:
    """
    Manages owl animation lifecycle.
    ...
    def __init__(
        ...
        self._current_state: Optional[OwlState] = None
    ...
    @property
    def current_state(self) -> Optional[OwlState]:
    ...
    async def start(self, state: OwlState) -> None:
        """
        ...
        from aios.cli.owl.state import OwlStateMapper
        if OwlStateMapper.should_animate(state):
            self._task = asyncio.create_task(self._animation_loop())
        else:
            # Static state - just set state, no frame rendering needed
            self._task = None

    async def change_state(self, new_state: OwlState) -> None:
        """
        ...
        from aios.cli.owl.state import OwlStateMapper
        if not OwlStateMapper.is_interruptible(self._current_state, new_state):
    ...
    async def complete(self, result: str = "PASS") -> None:
        ...
        self._current_state = OwlState.COMPLETE
        self._animation_state = AnimationState.COMPLETING
        self._frame_index = 0
        self._start_time = time.monotonic()

        # Run the complete animation sequence
        animation = OwlSprites.get_animation(OwlSpriteState.COMPLETE)
        ...
        # Render IDLE state
        idle_frame = self.renderer.render_static(OwlState.IDLE)
    ...
    async def _animation_loop(self) -> None:
        ...
        try:
            while self._animation_state == AnimationState.RUNNING:
                ...
                animation = OwlSprites.get_animation(OwlSpriteState(self._current_state.value))
...
class SyncOwlAnimator:
    ...
    def __init__(self, renderer: OwlRenderer):
        self._renderer = renderer
        self._animator = OwlAnimator(renderer)

    def start(self, state: OwlState) -> None:
        """Sync start works."""
    ...
    def change_state(self, new_state: OwlState) -> None:
    ...
    def animate_complete_sync(self, result: str = "PASS", frame_callback: Optional[Callable[[str], None]] = None) -> None:
        ...
        try:
            ...
            idle = self._renderer.render_static(OwlState.IDLE)
            callback(idle)


# Import time for SyncOwlAnimator
import time