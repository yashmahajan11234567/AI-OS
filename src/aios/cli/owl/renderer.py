...
from aios.cli.owl.sprites import OwlSprites, OwlSpriteState, OwlFrame
from aios.cli.owl.palette import OwlPalette, DEFAULT_PALETTE, MONOCHROME_PALETTE
from aios.cli.owl.state import OwlState
from aios.cli.owl.halfblock import RenderMode, HalfBlockRasterizer, render_fallback, render_json
...
class OwlRenderer:
    """
    Renders owl frames to terminal with capability detection and fallbacks.
    ...
    # Minimum width for full owl rendering
    MIN_FULL_WIDTH = 40
    MIN_NARROW_WIDTH = 20
    ...
    def __init__(
        ...
        palette: Optional[OwlPalette] = None,
    ):
    ...
    def get_palette(self) -> OwlPalette:
    ...
    def render_static(
        ...
        """
        Render a static owl frame for the given state.
        ...
        # Map OwlState to OwlSpriteState
        sprite_state = OwlSpriteState(state.value)

        if mode == RenderMode.NARROW:
            frame = OwlSprites.get_static_frame(sprite_state, narrow=True, monochrome=False)
            return "\n".join(frame.lines)
        elif mode == RenderMode.MONOCHROME:
            frame = OwlSprites.get_static_frame(sprite_state, narrow=False, monochrome=True)
            return "\n".join(frame.lines)
        elif mode == RenderMode.FALLBACK:
            return render_fallback(state.value)
        else:
            frame = OwlSprites.get_static_frame(sprite_state, narrow=False, monochrome=False)
            return "\n".join(frame.lines)
    ...
    def render_animation_frame(
        ...
        sprite_state = OwlSpriteState(state.value)
        animation = OwlSprites.get_animation(sprite_state)
    ...
    def render_startup_screen(
        ...
        """Render the AI-OS startup screen with owl and status info."""
        ...
        # Get IDLE owl frame
        owl_text = self.render_static(OwlState.IDLE)

        # Build status lines
        lines = owl_text.split('\n')

        # Add status panel to the right of owl
        status_lines = [
        ...
        # Combine owl with status
        max_owl_lines = len(lines)
        result_lines = []
        for i in range(max(max_owl_lines, len(status_lines))):
            owl_line = lines[i] if i < max_owl_lines else ""
            status_line = status_lines[i] if i < len(status_lines) else ""
            result_lines.append(f"{owl_line}{status_line}")
