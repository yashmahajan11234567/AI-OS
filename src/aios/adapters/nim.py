"""
NVIDIA NIM Provider for AI-OS ModelRouter - M14-T2 Implementation.

Adds NVIDIA NIM as a provider/backend to the EXISTING ModelRouter.
Does NOT create another ModelRouter or parallel model abstraction.

NVIDIA NIM is PRODUCTION READY with proper authentication.
...
class NimConfig:
    ...
    base_url: str = "https://integrate.api.nvidia.com/v1"  # Default NVIDIA NIM endpoint
    api_key: str | None = None
    timeout_seconds: int = 30
    default_model: str = "nemotron-3-8b-chat"  # Common NVIDIA model


class NimProvider(Provider):
    """NVIDIA NIM provider for ModelRouter.

    This provider integrates NVIDIA NIM as a model backend behind the
    ...
    def _classify_nim_error(self, exception: Exception) -> ProviderFailure:
        """
        Classify NVIDIA NIM errors into structured failure categories.

        Args:
            exception: The exception from the NIM API call (may have retry_after attribute)
        ...
        if "error " in error_str and ":" in error_str:
            try:
                # Extract status code from messages like "NVIDIA NIM error 401: ..."
    ...
    async def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        try:
            ...
            # Map ModelRequest to NVIDIA NIM format (OpenAI-compatible)
            payload = {
            ...
            async with self._session.post(
                ...
                if resp.status != 200:
                    ...
                    error = RuntimeError(f"NVIDIA NIM error {resp.status}: {error_text}")
                ...
                provider_registry.update_provider_health("nim", healthy=True)
                ...
                return ModelResponse(
                    ...
                    cost=0.0,  # NIM pricing is handled externally
            ...
            failure = self._classify_nim_error(e)

            # Update provider health based on failure classification
            provider_registry = get_provider_registry()
            provider_registry.update_provider_health_from_failure("nim", failure)

            return ModelResponse(
                content=f"NVIDIA NIM error: {e}",
                model_id=request.preferred_model or self._config.default_model,
                provider=ModelProvider.NIM,
                tokens_used={},
                cost=0.0,
...
def register_nim_provider(
    model_router,
    config: NimConfig | None = None,
) -> NimProvider:
    """Register NVIDIA NIM provider with the ModelRouter.

    Registers the NimProvider with the ProviderRegistry and updates
    ...
    Args:
        ...
        config: Optional NIM configuration

    Returns:
        NimProvider instance
    """
    provider = NimProvider(config)

    # Register NIM model in the existing ModelRouter
    model_config = ModelConfig(
        model_id="nim-default",
        provider=ModelProvider.NIM,
        ...
        enabled=True,
        config={
            "provider": "nim",  # Reference provider by ID in registry
            "nim": True,        # Marker for NIM-specific handling
        },
    )
    ...
    if provider_registry is None:
        provider_registry = get_provider_registry()
    provider_registry.register_provider("nim", provider)

    return provider


def get_nim_config_from_env() -> NimConfig:
    """Get NVIDIA NIM configuration from environment variables."""
    return NimConfig(
