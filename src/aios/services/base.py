...
class BaseService(ABC):
    ...
    Contract enforced by convention:
      * Services obtain the shared event bus from ``get_event_bus()`` (the
        Kernel registers a single global bus before services start).
    ...
    def __init__(self, event_bus: EventBus | None = None, info: ServiceInfo | None = None):
        ...
        try:
            ...
            for sid in self._subscription_ids:
                try:
                    ...
                    logger.debug("Failed to unsubscribe %s", sid)
    ...
    def subscribe(
        ...
        sid = self._event_bus.subscribe(handler, event_types, filter_fn)
