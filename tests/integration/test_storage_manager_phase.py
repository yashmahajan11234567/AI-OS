"""
Task 11 — StorageManager Core Manager integration tests (Part 4 §4.5 / §4.2.3).

Verifies, in an integration context:

  * StorageManager is constructed by the kernel after Core Components C1–C4.
  * StorageManager is registered with LifecycleManager (Phase 2, "State & Storage")
    and driven by its phase topology — NOT by the engineering-service
    ``_start_services()`` loop.
  * StorageManager registers with the canonical ServiceRegistry (C2) as
    ``core.storage`` with core-manager metadata (Part 4 §4.5.11 names
    kernel.storage; INV-SR-NS-002 reserves the kernel namespace; core.storage
    follows the core.lifecycle / core.state precedent).
  * StorageManager consumes the frozen ConfigurationManager (C3) and logs
    through the StructuredLogger (C4).
  * Shutdown runs LifecycleManager's reverse-phase order and marks
    ``core.storage`` SHUTDOWN in the canonical registry.
  * StorageManager and StateManager coexist (both Phase 2, alphabetically ordered).
...
@pytest.mark.asyncio
async def test_kernel_registers_storage_manager_as_phase2():
    k = await _start_kernel()
    try:
        await k.start()
        # 1. StorageManager constructed by the kernel after C1–C4.
        assert isinstance(k.storage_manager, StorageManager)
        # 2. Registered with LifecycleManager for Phase-2 orchestration.
        assert k.lifecycle is not None
        assert "StorageManager" in k.lifecycle._managers
        # 3. Driven by LifecycleManager -> initialized.
        assert k.storage_manager.is_initialized
        assert k.storage_manager.health_ready() is True
        # 4. Same instance as the global singleton and as the kernel-held one.
        assert k.storage_manager is get_storage_manager()
    finally:
        await k.stop()
    _reset_all()
...
@pytest.mark.asyncio
async def test_kernel_storage_registered_in_canonical_sr_as_core_storage():
    k = await _start_kernel()
    try:
        await k.start()
        ...
        assert reg.service is k.storage_manager
...
@pytest.mark.asyncio
async def test_storage_manager_not_in_start_services_path():
    k = await _start_kernel()
    try:
        await k.start()
        # StorageManager's lifecycle is owned by LifecycleManager; it must NOT
        # appear among the kernel's started engineering services.
        assert "storage_manager" not in k._services
...
@pytest.mark.asyncio
async def test_storage_manager_uses_canonical_sr_cm_sl():
    k = await _start_kernel()
    try:
        await k.start()
        sm = k.storage_manager
...
@pytest.mark.asyncio
async def test_storage_manager_health_ready_after_kernel_start():
    k = await _start_kernel()
    try:
        await k.start()
        assert k.storage_manager.health_ready() is True
...
@pytest.mark.asyncio
async def test_storage_manager_shutdown_marks_core_storage_shutdown():
    k = await _start_kernel()
    await k.start()
    await k.stop()
    # After kernel stop, LifecycleManager shutdown ran reverse phase order, which
    # marked core.storage SHUTDOWN in the canonical registry.
    reg = k.service_registry.get_registration("core.storage")
    assert reg is not None
    assert reg.lifecycle_state.value == "SHUTDOWN"
    assert k.storage_manager.health_ready() is False
    _reset_all()


@pytest.mark.asyncio
async def test_storage_manager_coexists_with_state_manager():
    """Both Phase-2 Core Managers are constructed and driven by LifecycleManager."""
    k = await _start_kernel()
    try:
        await k.start()
        ...
        assert k.storage_manager.health_ready() is True
        # Both registered in the canonical ServiceRegistry.
        sr = k.service_registry
...
@pytest.mark.asyncio
async def test_storage_manager_business_apis_work_after_kernel_start():
    k = await _start_kernel()
    try:
        await k.start()
        sm = k.storage_manager
        obj = sm.store_artifact("kernel-artifact", {"data": "test"})
        assert sm.retrieve_artifact("kernel-artifact") is obj
        # StateManager still works alongside StorageManager.
        k.state_manager.set_state(StateScope.WORKFLOW, "wf-1", "x", 1)
        assert k.state_manager.get_state(StateScope.WORKFLOW, "wf-1", "x") == 1
    finally:
        await k.stop()
    _reset_all()
...
@pytest.mark.asyncio
async def test_storage_manager_namespaces_available_after_kernel_start():
    k = await _start_kernel()
    try:
        await k.start()
        sm = k.storage_manager
        assert sm.namespaces == list(StorageNamespace)
...
@pytest.mark.asyncio
async def test_storage_manager_singleton_identity_after_kernel_start():
    k = await _start_kernel()
    try:
        await k.start()
        # The kernel-held instance IS the global singleton.
        assert k.storage_manager is get_storage_manager()
...
def test_no_task_1_to_9_to_10_regressions_imports_resolve():
