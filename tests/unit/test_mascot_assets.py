"""
Unit tests for MascotAssets runtime module.
...
class TestMascotAssets:
    """Tests for MascotAssets runtime module."""
    ...
    def test_asset_loading_deterministic(self):
        ...
        for state in self.STATES:
            frames1 = MascotAssets.get_frames(state)
            frames2 = MascotAssets.get_frames(state)
    ...
    def test_get_frame_index_wrapping(self):
        ...
        for state in self.STATES:
            ...
            if frame_count > 1:
                frame0 = MascotAssets.get_frame(state, 0)
                frame_wrapped = MascotAssets.get_frame(state, frame_count)
    ...
    def test_get_frame_count(self):
        ...
        for state, expected in expected_counts.items():
            assert MascotAssets.get_frame_count(state) == expected, f"Frame count mismatch for {state}"

    def test_get_dimensions(self):
        """Dimensions must be consistent for each state."""
        for state in self.STATES:
            w, h = MascotAssets.get_dimensions(state)
            assert w > 0 and h > 0
    ...
    def test_verify_all(self):
        ...
        assert MascotAssets.verify_all(), "Global asset verification failed"

    def test_unknown_state_returns_idle(self):
        """Unknown state should fall back to IDLE."""
        frames = MascotAssets.get_frames("UNKNOWN_STATE")
        idle_frames = MascotAssets.get_frames("IDLE")
