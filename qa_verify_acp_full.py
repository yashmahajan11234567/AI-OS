#!/usr/bin/env python3
"""QA verification script WITH child stderr capture to a file."""
...
# Enable debug logging so AcPAdapter._read_stderr logs the child's stderr
import logging
logging.basicConfig(level=logging.DEBUG, format="ADAPTER_LOG: %(message)s")
...
async def main():
    with tempfile.TemporaryDirectory(prefix="acp_test_") as tmp:
        ...
        child_stderr_file = tmp / "child_stderr.log"
...
''')
        ...
        try:
            ...
            print(f"VERIFY: new_session() returned ID of length {len(session_id)}")
            ...
            print(f"VERIFY: prompt() text starts with = {prompt_result.get('text', '')[:40]}...")
            print(f"VERIFY: prompt() sessionId in result = {prompt_result.get('sessionId', '')[:8]}...")
